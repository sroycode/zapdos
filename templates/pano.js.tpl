L.PanoBase = L.Class.extend({
    ...
    featureToPopupContent: function (feature) {
        var container = L.DomUtil.create('div', 'leaflet-pano-popup'),
...
L.PanoSearch = L.PanoBase.extend({

    includes: L.Mixin.Events,

    options: {
        url: 'http://YOUR_WEBSITE/api/?',
        ...
        feedbackEmail: 'sroycode@gmail.com',  // Set to null to remove feedback box
    ...
    createResultsContainer: function () {
        this.resultsContainer = L.DomUtil.create('ul', 'pano-autocomplete', document.querySelector('body'));
    },
    ...
    _formatResult: function (feature, el) {
        ...
        if (type && type !='yes') details.push(type);
        if (feature.properties.city && feature.properties.city !== feature.properties.name) {
        	if (feature.properties.locality) details.push(feature.properties.locality);
    ...
    handleResults: function(geojson) {
        ...
        if (geojson.features.length === 0) {
            var noresult = L.DomUtil.create('li', 'pano-no-result', this.resultsContainer);
            noresult.innerHTML = this.options.noResultLabel;
        }
        if (this.options.feedbackEmail) {
            var feedback = L.DomUtil.create('a', 'pano-feedback', this.resultsContainer);
...
L.Control.Pano = L.Control.extend({
    ...
    onAdd: function (map, options) {
        this.map = map;
        this.container = L.DomUtil.create('div', 'leaflet-pano');

        this.options = L.Util.extend(this.options, options);

        this.input = L.DomUtil.create('input', 'pano-input', this.container);
        this.search = new L.PanoSearch(map, this.input, this.options);
...
L.Map.addInitHook(function () {
    if (this.options.panoControl) {
        this.panoControl = new L.Control.Pano(this.options.panoControlOptions || {});
        this.addControl(this.panoControl);
    }
});

L.PanoReverse = L.PanoBase.extend({

    includes: L.Mixin.Events,

    options: {
        url: 'http://pano.justdis.com/reverse/?',
        limit: 1,
        handleResults: null
    },
